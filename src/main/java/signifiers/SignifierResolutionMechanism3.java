...
public class SignifierResolutionMechanism3 extends Agent {
    ...
    */
    ...
    /* Getting ability from an action
    Search for the signifier that exposes the action and get ability from there
    */
    public List<Ability> getPlanAbilities(Action plan_action){

        PredicateIndicator predicate_indicator = new PredicateIndicator("signifier", 3);

        Iterator<Literal> candidate_beliefs = getBB().getCandidateBeliefs(predicate_indicator);

        List<Ability> plan_abilities_list = new ArrayList<Ability>();

        if(candidate_beliefs == null){
            return plan_abilities_list;
        }
        ...
        while (candidate_beliefs.hasNext()){
            ...
            if (plan_action.equals(exposed_action)){
                List<Term> abilities_list = (List<Term>) candidate_belief.getTerm(2);

                for (Term ability: abilities_list){
                    plan_abilities_list.add(new Ability(ability));
                }
        ...
        if (options != null && !options.isEmpty()) {
            ...
            for (Option option: options) {
                ...
                if (!exposed_actions.containsAll(plan_actions)){
                    ...
                    /* Creating a set of plan abilities and checking if it is a subset of exposed abilities */
                    for (Action action: plan_actions){
                        List<Ability> recommended_abilities_list = getPlanAbilities(action);
                        System.out.println("Recommended abilities for " + action +": " + recommended_abilities_list);
                        for(Ability recommended_ability: recommended_abilities_list){
                            if(!agent_abilities.contains(recommended_ability)){
                                System.out.println("The recommended ability " + recommended_ability + " is not available, skipping plan...\n");
                                is_applicable = false;
                                break;
                            }
                        }
